...
namespace Dispatch.Scripts.DevKit.OrderUpdateScript.Examples.AddHubBasedOnPostalCode
{
    public class AddHubBasedOnPostalCode : IOrderUpdateScript
    {
        public async Task OnOrderUpdate(IOrderUpdater order, IScriptDataProvider data, ILogger logger)
        {
            ...
            if (account is null || !accountNumbers.Any(x => string.Equals(x?.Trim(), account.Number, StringComparison.InvariantCultureIgnoreCase)))
            {
                return;
            }
