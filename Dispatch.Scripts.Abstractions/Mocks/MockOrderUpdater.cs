...
{
    public class MockOrderUpdater : IOrderUpdater
    {
        ...
        public MockOrderUpdater(ScriptDebugWrapper scriptDebugWrapper, ILogger logger, string? orderId = null)
        {
            ...
            foreach (var group in _orderReader.OrderGroups)
            {
                group.SetDebugWrapper(scriptDebugWrapper);
            }
        }
        ...
        public IOrderGroupUpdater[] OrderGroups => _orderReader.OrderGroups.Select(x => new MockOrderGroupUpdater(x, _scriptDebugWrapper, _logger)).ToArray();
        IOrderGroupReader[] IOrderReader.OrderGroups => OrderGroups;
        ...
        {
            await Task.CompletedTask;
            _logger.LogInformation($"[{OrderId}] {nameof(AddToGroup)}('{groupId}') called");
        }
        ...
        internal class MockOrderGroupUpdater : IOrderGroupUpdater
        {
            private readonly IOrderGroupReader _reader;
            private readonly ScriptDebugWrapper _debugWrapper;
            private readonly ILogger _logger;

            public MockOrderGroupUpdater(IOrderGroupReader reader, ScriptDebugWrapper debugWrapper, ILogger logger)
            {
                _reader = reader;
            ...
            public string Id => _reader.Id;
            public string Key => _reader.Key;

            public async Task Close()
            {
                await Task.CompletedTask;
                _logger.LogInformation($"[OrderGroup {Id}] {nameof(Close)}() called");
            }

            public async Task RemoveOrder(string orderId)
            {
                await Task.CompletedTask;
                _logger.LogInformation($"[OrderGroup {Id}] {nameof(RemoveOrder)}('{orderId}') called");
            }

            public async Task SetAsMasterOrder(string orderId)
            {
                await Task.CompletedTask;
                _logger.LogInformation($"[OrderGroup {Id}] {nameof(SetAsMasterOrder)}('{orderId}') called");
            }

            Task<IOrderGroupDetailReader> IOrderGroupReader.GetDetail() => GetDetailInternal();

            public async Task<IOrderGroupDetailUpdater> GetDetail() => (IOrderGroupDetailUpdater)await GetDetailInternal();

            private async Task<IOrderGroupDetailReader> GetDetailInternal()
            {
                await Task.CompletedTask;
                var detail = _debugWrapper.GetOrderGroupDetail<MockOrderGroupDetailUpdater>(Id);
                if (detail is not null)
                {
                    detail.SetContext(_debugWrapper, _logger);
                }
                return detail;
            }
        }

        public class MockOrderGroupDetailUpdater : IOrderGroupDetailUpdater
        {
            private ScriptDebugWrapper _debugWrapper = default!;
            ...
            public MockOrderGroupItemUpdater[] Items { get; set; } = [];

            IOrderGroupItemUpdater[] IOrderGroupDetailUpdater.Items => Items;
            IOrderGroupItemReader[] IOrderGroupDetailReader.Items => Items;

            public void SetContext(ScriptDebugWrapper debugWrapper, ILogger logger)
            {
                _debugWrapper = debugWrapper;
                _logger = logger;
                foreach (var item in Items)
                {
                    item.SetContext(debugWrapper, logger);
                }
            }
        }

        public class MockOrderGroupItemUpdater : IOrderGroupItemUpdater
        {
            private ScriptDebugWrapper _debugWrapper = default!;
            private ILogger _logger = default!;
            ...
            public async Task<IOrderUpdater> GetOrder()
            {
                await Task.CompletedTask;
                return new MockOrderUpdater(_debugWrapper, _logger, OrderId);
            }
        }
    }
}
